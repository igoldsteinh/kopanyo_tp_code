...
# Secondary simulation setting "increase sampling window" as described in manuscript
           ...
           map(~.x %>% mutate(max_time = max(out.time[!is.na(out.time)]),
            ...
            map(~.x %>% mutate(active_start = ceiling(inf.time + t_incub),
                                             ...
                                             last_three = new_out >= max_time - 35 & active_start < max_time - 23,
                                              last_four = new_out >= max_time - 47 & active_start < max_time - 35,
                                              last_five = new_out >= max_time - 59 & active_start < max_time - 47,
                                              last_six = new_out >= max_time - 71 & active_start < max_time - 59,
                                              last_seven = new_out >= max_time - 83 & active_start < max_time - 71))
...
filtered_samp_four <- map(outputs, ~.x %>% filter(last_four == TRUE & active_time > 0) %>%
                             mutate(year_start = max_time - 47,
                                    year_end = max_time - 36))

filtered_samp_five <- map(outputs, ~.x %>% filter(last_five == TRUE & active_time > 0) %>%
                            mutate(year_start = max_time - 59,
                                   year_end = max_time - 48))

filtered_samp_six <- map(outputs, ~.x %>% filter(last_six == TRUE & active_time > 0) %>%
                            mutate(year_start = max_time - 71,
                                   year_end = max_time - 60))

filtered_samp_seven <- map(outputs, ~.x %>% filter(last_seven == TRUE & active_time > 0) %>%
                           mutate(year_start = max_time - 83,
                                  year_end = max_time - 72))
...
num_infectors_four <- map(filtered_samp_four, dim) %>%
  map(1)

num_infectors_five <- map(filtered_samp_five, dim) %>%
  map(1)

num_infectors_six <- map(filtered_samp_six, dim) %>%
  map(1)

num_infectors_seven <- map(filtered_samp_seven, dim) %>%
  map(1)
...
num_samples_four <- map(num_infectors_four, ~round(.x*.16))
num_samples_five <- map(num_infectors_five, ~round(.x*.16))
num_samples_six <- map(num_infectors_six, ~round(.x*.16))
num_samples_seven <- map(num_infectors_seven, ~round(.x*.16))
...
filtered_samp_four <- map2(filtered_samp_four, random_samp_one, ~.x %>%
                              filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_two, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
map2(random_samp_three, ~.x %>%
       filter(!(hosts.ID %in% .y$hosts.ID)))

random_samp_four <- map2(filtered_samp_four, num_samples_four, sample_n)

filtered_samp_five <- map2(filtered_samp_five, random_samp_one, ~.x %>%
                             filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_two, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_three, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_four, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID)))


random_samp_five <- map2(filtered_samp_five, num_samples_five, sample_n)

filtered_samp_six <- map2(filtered_samp_six, random_samp_one, ~.x %>%
                             filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_two, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_three, ~.x %>%
  ...
  map2(random_samp_four, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_five, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID)))


random_samp_six <- map2(filtered_samp_six, num_samples_six, sample_n)

filtered_samp_seven <- map2(filtered_samp_seven, random_samp_one, ~.x %>%
                            filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_two, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_three, ~.x %>%
  ...
  map2(random_samp_five, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID))) %>%
  map2(random_samp_six, ~.x %>%
         filter(!(hosts.ID %in% .y$hosts.ID)))


random_samp_seven <- map2(filtered_samp_seven, num_samples_seven, sample_n)


random_samp <- pmap(list(random_samp_one, 
                         random_samp_two, 
                         random_samp_three,
                         random_samp_four,
                         random_samp_five,
                         random_samp_six,
                         random_samp_seven), rbind)
