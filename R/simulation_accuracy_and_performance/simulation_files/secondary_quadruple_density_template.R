...
# Secondary simulation setting "quadruple sampling density" as described in manuscript
# Run for seeds 1 to 520 to replicate manuscript analysis
library(nosoi)  
library(tidyverse)
library(TransPhylo)
library(ape)
...
num_samples_one <- map(num_infectors_one, ~round(.x*.64))
num_samples_two <- map(num_infectors_two, ~round(.x*.64))
num_samples_three <- map(num_infectors_three, ~round(.x*.64))
...
write_rds(results, str_c("secondary_quadruple_density_res_seed_", seed, ".rds", ""))
