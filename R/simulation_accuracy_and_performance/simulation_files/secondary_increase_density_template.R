...
# Run for seeds 1 to 105 to replicate manuscript analysis
library(nosoi)  
library(tidyverse)
library(TransPhylo)
library(ape)
...
num_samples_one <- map(num_infectors_one, ~round(.x*.32))
num_samples_two <- map(num_infectors_two, ~round(.x*.32))
num_samples_three <- map(num_infectors_three, ~round(.x*.32))
...
# read in the results -----------------------------------------------------
res_name_suffix <- list("16active_8yearsim_1.75diff_dubclusters_res_seed_"
)
...
setting <- list(
  "16active_8yearsim_1.75diff_dubclusters"
...
write_rds(acc, "dubclusters_1.75diff_acc_tablesv1.rds")
write_rds(test_summary, "dubclusters_1.75diff_sim_or_tablesv1.rds")
