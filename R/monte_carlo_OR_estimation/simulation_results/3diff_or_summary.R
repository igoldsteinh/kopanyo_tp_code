...
# seed_list <- as.list(as.character(c(58,60)))
res_name_suffix <- "16active_8yearsim_3diff_odds_seed_"
...
# 
# res_list <- map(seed_list, ~map(.x, ~read_rds(here::here("code",
#                                            "R Code",
#                                            "odds_estimation_files",
#                                            .x))))

# res_list <- map(res_list, ~map(.x, ~read_rds(.x)))
...
write_rds(summary_odds, "summaryodds_3diff_1milv2.rds")